...
$(document).ready(function(){
  ...
  $("#favoritesAllBtn").click(function(event) {
    ...
    $.get(`https://my-sweater-weather.herokuapp.com/api/v1/favorites?api_key=${document.cookie}`, function(data, status) {
      ...
      if(favorited[4]) {
        ...
        $(".fav1").text("No current favorites!")
      }
    })
  });
  $("#deleteFavBtn").click(function(event) {
    debugger;
    event.preventDefault();
    fetch(`https://my-sweater-weather.herokuapp.com/api/v1/favorites?api_key=${document.cookie}&location=${event.target.location}`, {
      method: 'DELETE',
      headers: { "Content-Type": "application/json" },
    }).then(response => response.json())
    .catch(error => console.error({ error }))
    alert("You have deleted a favorite!")
  })
